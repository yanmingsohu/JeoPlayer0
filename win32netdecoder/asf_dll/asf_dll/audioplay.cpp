							  ...
							  /* [in] */ void __RPC_FAR *pvContext )
{
	...
	LPWAVEHDR pwh;
	DWORD maxlen;
	pSample->GetMaxLength( &maxlen );
	getSoundBuffer(&pwh, maxlen);
	...
	changePcmVol( pData, cbData );
	processing( pData, cbData );
...
HRESULT CAudioPlay::Init()
{
	...
	do
	{
		...
		{
			_Tr( "Could not create the event" );
			hr = E_FAIL;
			break;
		}
...
HRESULT CAudioPlay::Open( LPCWSTR pwszUrl )
{
	...
	do
	{
		...
		if( FAILED( hr ) )
        {
            _Tr( "Could not open the specified file" );
			break;
		}
            ...
            fAcquiringLicenseNonSilently )
		{
			...
			//
			MessageBox( g_hwndDialog, _Tr( "After acquiring the license, re-open the file." ), ERROR_DIALOG_TrITLE, MB_OK );
		...
		if( FAILED (hr) )
		{
			_Tr( "Could not QI for IWMHeaderInfo" );
			break;
		}
    ...
    while( 0 != GetMessage( &uMsg, NULL, 0, 0 ) )
    {
		...
		case WOM_DONE:
			...
			else
			{
				...
				_Tr("Wave function (waveOutUnprepareHeader) failed");
			}
...
HRESULT CAudioPlay::GetAudioOutput()
{
	...
	do
	{
		...
		if( FAILED( hr ) )
		{
			_Tr( "Could not get output count" );
			break;
		}
		...
		for( i = 0; i < cOutputs; i++ )
		{
			...
			if( FAILED( hr ) )
			{
				_Tr( "Could not get output props" );
				break;
			}
			...
			if( FAILED( hr ) )
			{
				_Tr( "Could not query for the space needed for media type" );
				break;
			}
		...
		if( i == cOutputs )
		{
			...
			_Tr( "Could not find an audio stream in the specified file" );
			break;
		}
	...
	if( FAILED( hr ) )
	{
       _Tr("get audio output error");
	}

	printf("radio sample rate: %d\n", m_pWfx->nSamplesPerSec);
	setSampleRate( m_pWfx->nSamplesPerSec );
...
HRESULT CAudioPlay::RetrieveAndDisplayAttributes()
{
	...
	do
	{
		...
		if( FAILED( hr ) )
		{
			_Tr( "Could not get the Title attribute" );
			break;
		}
		...
		if( FAILED( hr ) )
		{
			_Tr( "Could not get the Author attribute" );
			break;
		}
		...
		if( FAILED( hr ) )
		{
			_Tr( "Could not get the Copyright attribute" );
			break;
		}
		...
		if( FAILED( hr ) )
		{
			_Tr( "Could not get the Duration attribute" );
			break;
		}
		...
		if( FAILED( hr ) )
		{
			_Tr( "Could not get the Seekable attribute" ) ;
			break;
		}
		...
		if( FAILED( hr ) )
		{
			_Tr( "Could not get the Broadcast attribute" );
			break;
		}
...
void CAudioPlay::SetVolume(DWORD volume) {
	if (volume>0x0ffff) volume = 0x0ffff;
...
void SetCurrentStatus( AUDIOSTATUS currentStatus ) {
	extern AUDIOSTATUS currstate;
	currstate = currentStatus;
}
